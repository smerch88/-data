...
AWS Bedrock global vs regional endpoints for Opus 4.7, Haiku 4.5, and newer models, or the legacy integration for all other models on Bedrock
Google Vertex AI global, multi-region, and regional endpoints
...
For implementation details, supported models, and code examples, see the prompt caching documentation.
...
This applies to the Claude API (1P) only. Third-party platforms have their own regional pricing. See AWS Bedrock and Google Vertex AI for details. Earlier models retain their existing pricing regardless of inference_geo settings.

For more information, see the data residency documentation.
...
For more information, see the fast mode documentation.
...
For more information about batch processing, see the batch processing documentation.
...
For more information about tool use implementation and best practices, see the tool use documentation.
...
All tokens consumed by a Claude Managed Agents session are billed at the rates shown in Model pricing. Prompt caching multipliers apply identically. Web search triggered inside a session incurs the standard $10 per 1,000 searches.
...
Third-party platform pricing	Claude Managed Agents is available only through the Claude API directly.
Session runtime
SKU	Rate	Metering
